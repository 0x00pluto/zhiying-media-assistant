...
export function BatchBloggerPage({ initialState }: Props) {
  ...
  const [limit, setLimit] = useState(Number(initialState?.limit) || 100)
  ...
  useEffect(() => {
    if (initialState) {
      ...
      setLimit(Number(initialState.limit) || 100)
    }
  }, [initialState])
  ...
  const startTask = async () => {
    ...
    const condition: Record<string, unknown> = {
      name: (initialState?.name as string) || "博主批量采集",
      collectBy,
      limit
    }
    ...
    try {
      const task = runTask("blogger", condition, "/xiaohongshu/batch-collect/blogger")
  ...
  return (
    <div>
      <h2 style={{ marginTop: 0 }}>批量采集博主</h2>
      ...
      <label style={labelStyle}>
        ...
        <select
          ...
          <option value="links">博主链接</option>
        </select>
      </label>
      ...
      <div style={{ display: "flex", gap: 8, marginBottom: 16 }}>
        ...
        <button
          ...
          onClick={() => exportCsv(BLOGGER_COLUMNS, records, "小红书博主")}
          style={secondaryBtn}>
          导出 CSV
        </button>
        <button
          type="button"
          disabled={!records.length}
          onClick={() => copyToClipboard(BLOGGER_COLUMNS, records)}
          style={secondaryBtn}>
          复制
        </button>
      </div>
      ...
      {records.length > 0 && (
        <div style={{ overflow: "auto", maxHeight: 360, border: "1px solid #e5e7eb", marginTop: 12 }}>
          <table style={{ width: "100%", fontSize: 12, borderCollapse: "collapse" }}>
            <thead>
              <tr>
                {BLOGGER_COLUMNS.filter((c) => c.default).map((col) => (
