# AGENTS.md — 智赢媒体助手（quanmediacrawl）

供 AI Agent 快速理解本仓库并安全改动代码。阅读本文后再动手。

## 项目是什么

Chrome 扩展（Manifest V3），产品名 **智赢媒体助手**。从 `demo/社媒助手-v3.3.0` 迁移重构，当前以 **小红书** 为主，支持笔记/博主/评论采集、批量任务、导出、飞书多维表格同步。
...
specs/
├── prds/                   # PRD（正式产品需求，/team:product-manager）
└── features/               # Feature Spec（早期探索，/team:po-explorer）
```

**规格文档与团队命令**：PRD 见 `specs/prds/`；Feature Spec 见 `specs/features/`。Cursor 团队命令安装在 `.cursor/commands/team/`（含 `po-explorer`、`product-manager`、`plugin-engineer`、`prd-accept`；`.cursor` 在 `.gitignore` 中，默认不入库）。
...
| 场景 | 文件 |
|------|------|
| Feature Spec 目录 | `specs/features/README.md` |
| PRD 目录 | `specs/prds/README.md` |
